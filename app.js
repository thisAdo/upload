(function() {
    ...
    function getFileIcon(filename, isFolder) {
        ...
        const color = iconColors[ext] || '#8c8c8c';
    ...
    function renderFileTree(files) {
        ...
        files.forEach(file => {
            ...
            item.className = 'file-item' + (file.folder ? ' folder' : ' file');
            
            const icon = getFileIcon(file.name, file.folder);
            const size = file.folder ? '' : `<span class="file-size-tree">${formatFileSize(file.size || 0)}</span>`;
            
            item.innerHTML = `${icon}<span class="file-name-tree">${file.path}</span>${size}`;
